...
Although advanced technical solutions for mitigating privacy risks in genetic data are still largely in the research and development phase, organizations are currently prioritizing privacy policies and data management practices that define privacy controls for genetic data (including, but not limited to, de-identification), while considering genetic data as not easily identifiable[(Future of Privacy Forum, 2020)](references.html#Future_of_Privacy_Forum_2020). Below are the typical privacy policies: 
- **Controlled Access:** Genetic data access is tiered based on re-identification risk, with open access repositories providing unrestricted public access to aggregated or summary data, while controlled access repositories limit individual-level data to approved research use. Access to controlled data requires pre-approval from a governing body, such as a Data Access Committee, which reviews requests for merit and compliance with data use limitations, granting either broad or study-specific access to authenticated researchers.
- **Contractual Controls:** Before accessing controlled genetic data, approved investigators and their institutions must sign a data use agreement to ensure legitimate use and mitigate re-identification risks, outlining appropriate data use and investigator obligations, such as restrictions on data redistribution, regular access reviews or renewals, prohibitions on re-identification attempts or contacting data subjects, and requirements to destroy data after study completion, with additional terms based on participant consent, data characteristics, or institutional policies.
- **Security Protocols** Investigators accessing open or controlled genetic data must follow security protocols set by the data-sharing organizations, which recommend local storage over cloud storage for sensitive data due to its greater security, despite risks in local systems. To mitigate these risks, organizations require physical and electronic security measures, including user training, access restrictions to authenticated users, and data encryption.
